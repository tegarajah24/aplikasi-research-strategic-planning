...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <div class="flex items-center justify-between flex-wrap gap-3">
            <div>
                <h1 class="text-xl font-bold text-slate-800 leading-tight">Program Studi</h1>
                <p class="text-sm text-slate-400 mt-0.5">Manajemen Data Program Studi</p>
            </div>
        </div>
     <?php $__env->endSlot(); ?>
    ...
    <div class="py-8 min-h-full" x-data="{ 
        ...
        editData: { id: '', kode_prodi: '', nama_prodi: '', fakultas_id: '' }
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $prodis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="py-4 px-6 text-sm text-slate-500 text-center border-r border-slate-100"><?php echo e($prodis->firstItem() + $index); ?></td>
                                <td class="py-4 px-6 text-sm font-medium text-slate-900 border-r border-slate-100"><?php echo e($item->kode_prodi); ?></td>
                                <td class="py-4 px-6 text-sm text-slate-700 border-r border-slate-100"><?php echo e($item->nama_prodi); ?></td>
                                <td class="py-4 px-6 text-sm text-slate-700 border-r border-slate-100">
                                    <?php echo e($item->fakultas ? $item->fakultas->nama_fakultas : '-'); ?>
                                ...
                                <td class="py-4 px-6 text-center">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <form action="<?php echo e(route('prodi.destroy', $item)); ?>" method="POST" class="inline-block" onsubmit="return confirm('Hapus data Program Studi ini?');">
                                            <?php echo csrf_field(); ?>
                            ...
                            <tr>
                                <td colspan="5" class="py-12 px-6 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <h3 class="text-sm font-medium text-slate-900">Belum ada data Program Studi</h3>
                ...
                <!-- Pagination -->
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($prodis->hasPages()): ?>
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                                    <h3 class="text-lg leading-6 font-semibold text-slate-900 mb-6 border-b pb-4">Tambah Data Program Studi</h3>
                                    
                                    <div class="space-y-4">
                                        <div>
                                            <label class="block text-sm font-medium text-slate-700">Kode Prodi</label>
                                            <input type="text" name="kode_prodi" required class="mt-1 block w-full border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-slate-700">Nama Program Studi</label>
                                            <input type="text" name="nama_prodi" required class="mt-1 block w-full border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-slate-700">Fakultas</label>
                                            <select name="fakultas_id" required class="mt-1 block w-full border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                                <option value="" disabled selected>-- Pilih Fakultas --</option>
                                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $fakultas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fak): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <option value="<?php echo e($fak->id); ?>"><?php echo e($fak->nama_fakultas); ?></option>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                            </select>
                                        </div>
                                    </div>
        ...
        <div x-show="showEditModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showEditModal" x-transition.scale.origin.bottom class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-slate-100">
                    <form :action="'<?php echo e(route('prodi.index')); ?>/' + editData.id" method="POST">
                        <?php echo csrf_field(); ?>
                        <?php echo method_field('PUT'); ?>
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                                    <h3 class="text-lg leading-6 font-semibold text-slate-900 mb-6 border-b pb-4">Edit Data Program Studi</h3>
                                    
                                    <div class="space-y-4">
                                        <div>
                                            <label class="block text-sm font-medium text-slate-700">Kode Prodi</label>
                                            <input type="text" name="kode_prodi" x-model="editData.kode_prodi" required class="mt-1 block w-full border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-slate-700">Nama Program Studi</label>
                                            <input type="text" name="nama_prodi" x-model="editData.nama_prodi" required class="mt-1 block w-full border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-slate-700">Fakultas</label>
                                            <select name="fakultas_id" x-model="editData.fakultas_id" required class="mt-1 block w-full border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\aplikasi-research-strategic-planning\resources\views\prodi\index.blade.php ENDPATH**/ ?>