...
// ── Helpers ──────────────────────────────────────────────────────
function totalPrograms() { return bidangData.reduce((s,b) => s + b.renstras.reduce((p,r) => p + r.programs.length, 0), 0); }
function totalKegiatan()  { return bidangData.reduce((s,b) => s + b.renstras.reduce((p,r) => p + r.programs.reduce((k,pr) => k + pr.kegiatan, 0), 0), 0); }
...
function renderTable() {
    ...
    tbody.innerHTML = filtered.map((b, i) => {
        const progCount = b.renstras.reduce((s, r) => s + r.programs.length, 0);
        const kgtCount = b.renstras.reduce((s, r) => s + r.programs.reduce((k, p) => k + p.kegiatan, 0), 0);
