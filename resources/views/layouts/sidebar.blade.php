...
<aside id="sidebar"
    ...
    <div class="flex items-center gap-3 px-5 py-5 border-b border-slate-800/70 flex-shrink-0 brand-container">
        <img src="https://uhb.ac.id/wp-content/uploads/2024/03/logo_UHB_r-1.png" alt="Logo UHB" class="w-9 h-auto object-contain">
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto" aria-label="Navigasi utama">
        ...
        {{-- Laporan RENOP --}}
        @php $isLaporanRenop = request()->is('laporan/renop*'); @endphp
        <a href="/laporan/renop"
           class="group relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium
                  transition-all duration-150
                  {{ $isLaporanRenop
                       ? 'bg-gradient-to-r from-blue-700 to-blue-600 text-white shadow-md shadow-blue-900/30'
                       : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}"
           aria-current="{{ $isLaporanRenop ? 'page' : 'false' }}">
            @if($isLaporanRenop)
                <span class="absolute left-0 top-1/2 -translate-y-1/2 h-4 w-[2px] bg-blue-300 rounded-full"></span>
            @endif
            <svg class="w-[18px] h-[18px] flex-shrink-0 {{ $isLaporanRenop ? 'text-blue-200' : 'text-slate-400 group-hover:text-slate-200' }} transition-colors"
                 fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
            </svg>
            <span>Laporan RENOP</span>
        </a>
...
<div class="lg:hidden fixed top-0 left-0 right-0 z-20 h-14 bg-white border-b border-slate-200
    ...
    <div class="flex items-center gap-2">
        <img src="https://uhb.ac.id/wp-content/uploads/2024/03/logo_UHB_r-1.png" alt="Logo UHB" class="w-6 h-auto object-contain">
